...
class CollectLIFO(object):
    ...
    #     last_readpos = min([max(positions or [self.write_pos]) for positions in self.read_pos.values()] or [0])
    #     return (self.N - (self.write_pos - last_readpos)) >= length
    ...
    def tokens_available(self, length, metadata):
        ...
        return len(self.tentative_read_pos[metadata]) > 0 
